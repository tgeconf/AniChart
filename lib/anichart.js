...
function () {
  ...
  _createClass(AniChart, [{
    ...
    value: function init(spec) {
      ...
      this.chartSpecs = [];
      var aniChartObj = spec; //deal with input charts
  ...
  _createClass(ChartSpec, null, [{
    ...
    value: function facetViews(nameCharts, facet) {
      if (facet.views.length > 0) {
        ...
        for (var i = 0; i < facet.views[0].length; i++) {
          var tmpRecorder = [];

          for (var j = 0; j < facet.views.length; j++) {
            var chartName = facet.views[j][i];
          ...
          if (ChartSpec.charts[i].querySelectorAll('#' + allMarks[_j3]).length === 0) {
            ...
            for (var a = 0; a < ChartSpec.changedAttrs.length; a++) {
              if (ChartSpec.changedAttrs[a] === 'y' && ChartSpec.changedAttrs.indexOf('height') >= 0) {
                var targetValue = parseFloat(mark.getAttribute('y')) + parseFloat(mark.getAttribute('height'));
    ...
    set: function set(views) {
      if (typeof views !== 'undefined') {
        ...
        for (var _i = 0; _i < views.length; _i++) {
          if (views[_i].length < maxLen) {
            var lastItem = views[_i][views[_i].length - 1];
            var numDiff = maxLen - views[_i].length;

            while (numDiff > 0) {
              views[_i].push(lastItem);
